...
export const ResponsabilePage: React.FC = () => {
    ...
    return (
        <div className="space-y-6">
            <div className="bg-white/60 backdrop-blur-xl p-4 rounded-2xl shadow-lg border border-gray-200">
                <div className="flex justify-between items-center flex-wrap gap-4">
                    <h2 className="text-xl font-bold text-gray-800">Responsabile Dashboard</h2>
                    <div className="flex items-center gap-4">
                         <button onClick={() => setShowUserManagement(true)} className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors border border-gray-300">
            ...
            {isLoading && (
                <div className="text-center p-8 bg-white/60 backdrop-blur-xl rounded-2xl shadow-lg border border-gray-200">
                    <p className="text-gray-600">Loading and analyzing data from Firebase...</p>
                </div>
            )}

            {!isLoading && error && (
                <div className="text-center p-8 bg-red-100 text-red-700 rounded-lg shadow-md border border-red-300">
            ...
            {!isLoading && !error && !analysisData && (
                <div className="text-center p-8 bg-white/60 backdrop-blur-xl rounded-2xl shadow-lg border border-gray-200">
                    <h2 className="text-2xl font-bold text-gray-800">Awaiting Files</h2>
                    <p className="mt-2 text-gray-600">The dashboard will be generated once the 'Magazzino' file and at least one 'Forza Vendita' file have been uploaded.</p>
                </div>
            )}

            {!isLoading && !error && analysisData && (
                <div className="bg-white/60 backdrop-blur-xl p-6 rounded-2xl shadow-lg border border-gray-200">
                            ...
                            <thead className="bg-gray-50">
                                <tr>
                                    {analysisData.length > 0 && Object.keys(analysisData[0]).map(key => (
                                        <th key={key} scope="col" className="px-6 py-3 text-left text-xs font-medium text-green-700 uppercase tracking-wider">{key}</th>
                                    ))}
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-200">
                                {analysisData.map((row, index) => (
                                    <tr key={index} className="odd:bg-white even:bg-gray-50 hover:bg-green-50/80 transition-colors">
                                        {Object.entries(row).map(([key, value]) => (
                                            <td key={key} className={`px-6 py-4 whitespace-nowrap text-sm text-gray-600 ${key === 'difference' && typeof value === 'number' && value < 0 ? 'text-red-600 font-bold' : ''}`}>
